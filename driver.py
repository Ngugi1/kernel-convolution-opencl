...
img = cv2.copyMakeBorder(image_to_array(sys.argv[1]), h_kernel_mid,h_kernel_mid,h_kernel_mid,h_kernel_mid,cv2.BORDER_CONSTANT,value=padding)
# Convert types 
...
d_input_image = cl.Buffer(context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=img.flatten())
d_kernel =  cl.Buffer(context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=h_kernel_flat)


# Create the memory on the device to put the result into.
d_output_image = cl.Buffer(context, cl.mem_flags.WRITE_ONLY, img.flatten().nbytes)
# Execute the kernel.
...
convolve.set_scalar_arg_dtypes([None, None,None,numpy.uint8,numpy.uint8, numpy.uint32, numpy.uint32, numpy.uint8])
# https://documen.tician.de/pyopencl/runtime_program.html?highlight=set_scalar_arg_dtypes#pyopencl.Kernel.__call__
convolve(queue,(img_h, img_w), None, d_input_image, d_output_image, d_kernel, h_kernel_size, h_kernel_mid,img_w,img_h, bytes_per_pixel)
